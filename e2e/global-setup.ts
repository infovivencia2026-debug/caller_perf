 ...
 */
const SEED_EMAILS = ["admin@example.com", "lakshmi@example.com", "arjun@example.com"];
...
export default async function globalSetup() {
  ...
  try {
    ...
    await client.query(
      `DELETE FROM "Customer" WHERE phone <> ALL($1::text[])`,
      [SEED_PHONES],
    );
    // Users too: an extra telecaller left behind by manual testing changes the
    // round-robin split below and makes the suite fail for reasons unrelated to the
    // code under test.
    await client.query(`DELETE FROM "User" WHERE email <> ALL($1::text[])`, [SEED_EMAILS]);
